...
- **Fixed slot map (positions never move by input):** icon/leadingView (leading 48 card / 44 row teal tile) · category (teal caps above title) · title · subtitle (short, directly under title) · status chip (**top-right, chip-only**) · body / bodyView (long text / structured content, full-width in the SAME left column as the title — never at the card edge) · warning (orange `exclamationmark.triangle` Label) · **Divider (component-drawn) → CTA/footer (bottom-left) + date (bottom-right)**.
- **Icon left, everything else in ONE content column** to its right — title, subtitle, body, warning, divider and meta all share the same left edge. `subtitle` and `body`/`bodyView` are distinct slots; neither shifts based on what else is present.
- **CTAs are chip-free:** all text CTAs go through `ctaTitle`/`ctaAction` (one style: teal `.subheadline.semibold`, bottom-left). Actions never sit in the top-right (that is chip-only). Bespoke meta content (metric/chart/progress) → `footerView`; structured body (chart, bullets, eat/limit rows) → `bodyView`.
- **Date is always bottom-right** in the meta row (rows: under the chip). One `date:` slot — never hand-roll a timestamp elsewhere.
...
- **Exempt (owner decision, leave hand-rolled):** Services `WorkoutsCardView` and `DoctorSectionView`. Not yet migrated: Feed section header, StatChip/WatchStatChip/HubCard, in-sheet cards.
