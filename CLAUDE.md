...
- **Date is always bottom-right** in the meta row (rows: under the chip). One `date:` slot — never hand-roll a timestamp elsewhere. Cards with no pill/date show a `Last updated/checked X ago` fallback in the same slot.
