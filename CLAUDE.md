...
- **Fixed slot map (positions never move by input):** icon/leadingView (leading 48 card / 44 row teal tile) · category (teal caps above title) · title · subtitle (short, directly under title) · status chip (**top-right, chip-only**) · body / bodyView (long text / structured content, full-width in the SAME left column as the title — never at the card edge) · date (**bare "X ago", left-aligned under the content — NO divider, NO label prefix**) · chevron (trailing, vertically-centred).
